...
<div class="container mx-auto p-6 bg-white shadow-md rounded-lg my-8">
    ...
    <form name="form-edit" id="form-edit" method="post" action="{{ route('issue.update', $issue->id)  }}"
          class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" id="description" name="description" value="{{ $issue->description }}"
                   class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"
                   required>
        </div>
    ...
    <form action="{{ route('issue.remove', ['id' => $issue->id]) }}" method="POST"
